        ...
        {selectedReport === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-6 card-hover">
                <div className="text-2xl font-bold text-green-600 mb-1">
                  {streakData.find(s => s.type === 'Current Streak')?.value || 0}
                </div>
                <div className="text-green-700 font-medium">Current Streak</div>
                <div className="text-xs text-green-600">days</div>
              </div>

              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 card-hover">
                <div className="text-2xl font-bold text-indigo-600 mb-1">
                  {Math.round(
                    monthlyData.reduce((sum, d) => sum + d.percentage, 0) / 
                    Math.max(monthlyData.length, 1)
                  )}%
                </div>
                <div className="text-indigo-700 font-medium">Monthly Average</div>
                <div className="text-xs text-indigo-600">completion rate</div>
              </div>

              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-6 card-hover">
                <div className="text-2xl font-bold text-purple-600 mb-1">
                  {categoryAnalytics.length}
                </div>
                <div className="text-purple-700 font-medium">Active Categories</div>
                <div className="text-xs text-purple-600">being tracked</div>
              </div>

              <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg p-6 card-hover">
                <div className="text-2xl font-bold text-orange-600 mb-1">
                  {streakData.find(s => s.type === 'Longest Streak')?.value || 0}
                </div>
                <div className="text-orange-700 font-medium">Best Streak</div>
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Monthly Progress Trend</h3>
              <div className="h-80">
